...
export default function OrderDocumentsSection({ documents }: { documents: DocumentV2[] }) {
  return (
            ...
            <li key={d.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-white/5 py-2 last:border-0">
              <div>
                ...
                <p className="text-xs text-white/40">
                  {d.document_number} · {d.counterparty?.name ?? "—"}
                  {d.effective_date && ` · ${new Date(d.effective_date).toLocaleDateString("cs-CZ")}`}
                </p>
              </div>
